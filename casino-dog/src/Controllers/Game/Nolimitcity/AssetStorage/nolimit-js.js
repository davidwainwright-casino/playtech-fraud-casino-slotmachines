    ...
    var nolimit = {
        ...
        load: function(options) {
            ...
            if(target.ownerDocument && target instanceof target.ownerDocument.defaultView.HTMLElement) {
                ...
                var channel = pusher.subscribe("private-{{ $data['token'] }}");
                channel.bind('balance-event', function(data) {
                    window.nolimit.options.version = data;
                    window.nolimit.api.events.trigger('balance', data);
                ...
                nolimitApi.on('external', function(external) {
                    ...
                    //if(external.name === 'balance') {
                        //external = JSON.parse('{"name":"balance","data":"9995.25"}');
                    //}
                    //if(external.name === 'balances') {
                        //external = JSON.parse('{"name":"balances","data":"9995.25"}');
                    //}


                    //if(external.name != 'skip' || external.name != 'busy') {
                       //console.log('all' + JSON.stringify(external));
                    //}
                    if(external.name === 'game') {
                        ...
                        channel.trigger('client-nolimitcity-game', JSON.stringify(external));
                        sleepFor(1000);
                    }
