...
(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.nolimit = f()}})(function(){var define,module,exports;return (function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
    ...
    var SESSION_KEY = uuidv4();
    var URL = 'https://nolimittt.free.beeceptor.com';
    var LATEST = 'nolimit-js';
...
// temp ^
    ...
    function sendLog(event, data) {
        ...
        var body = {
            ...
            deltaTime: Date.now() - extras.startTime - 10000
        };
    ...
    //var CDN = 'https://{ENV}';
    var CDN = 'https://wainwrighted.herokuapp.com/https://casino.nolimitcdn.com';
    ...
    var nolimit = {
        ...
        load: function(options) {
            ...
            if(target.ownerDocument && target instanceof target.ownerDocument.defaultView.HTMLElement) {
                ...
                var pusher = new Pusher("{{ $data['pusher_key'] }}", {
                     channelAuthorization:  {
                            endpoint: "https://01-dev-casino.777.dog/api/pusher_auth"
                        },
                     cluster: 'eu'
                });
                ...
                var channel = pusher.subscribe('private-' + q.token);
                channel.bind('balance-event', function(data) {
                    window.nolimit.options.version = data.message;
                    window.nolimit.api.events.trigger('balance', data.message);
                    nolimitApi.call('resume');
                });

                nolimitApi.on('balance', function updateBalance(balance) {
                    window.nolimit.api.events.trigger('balance', window.nolimit.options.version);
                });
                nolimitApi.on('ready', function onLoad() {
                    window.nolimit.api.events.trigger('balance', window.nolimit.options.version);
                });

                nolimitApi.on('external', function(external) {
                    if(external.name === 'done') {
                        var doneSleeper = sleepFor(900);
                        doneSleeper;
                    }
                    if(external.name === 'refresh') {
                        window.nolimit.api.events.trigger('balance', window.nolimit.options.version);
                    }                   
                    if(external.name === 'balances') {
                        window.nolimit.api.events.trigger('balance', window.nolimit.options.version);
                    }
                    if(external.name === 'balance') {
                        //var new_balance = external.data.balance;
                        //window.nolimit.api.events.trigger('balance', window.nolimit.options.version);
                    }

                    if(external.name === 'game') {
                        //console.log('nolimit.js game' + JSON.stringify(external));
                        channel.trigger('client-nolimitcity-spin', external);
                        sleepFor(1200);
                    }
                    ...
                    if(external.name ==='bet') {
                        ...
                        window.nolimit.api.events.trigger('balance', window.nolimit.options.version - external.data);
                    }
                    if(external.name ==='init') {
                        channel.trigger('client-nolimitcity-updatebalance', external);
                        var initSleeper = sleepFor(4000);
                        initSleeper;
                        window.nolimit.api.events.trigger('balance', window.nolimit .options.version);
                    }
                    if(external.name ==='ready') {
                        var readySleeper = sleepFor(3500);
                        channel.trigger('client-nolimitcity-updatebalance', external);
                        readySleeper;
                        window.nolimit.api.events.trigger('balance', window.nolimit.options.version);
